            ...
            item {
                Row(
                    ...
                    Card(
                        ...
                        Image(
                            painter = rememberAsyncImagePainter(
                                if (blog.author_image_uri.isEmpty()) {
                                    R.drawable.ic_pfp
                                } else {
                                    try {
                                        blog.author_image_uri
                                    } catch (e: Exception) {
                                        R.drawable.ic_pfp
                                    }
                                }
                    ...
                    Column(
                        ...
                        Text(text = blog.author_name, color = Color.Blue, fontSize = 24.sp)
                        Spacer(modifier = Modifier.height(5.dp))
                        Text(text = blog.date + " | " + blog.time, fontSize = 18.sp)
                    }
                }
            }
            item {
                Text(text = "Title - " + blog.title, fontSize = 24.sp)
                Spacer(modifier = Modifier.height(15.dp))
            }
            item {
                Text(text = blog.content, fontSize = 18.sp, textAlign = TextAlign.Justify)
                Spacer(modifier = Modifier.height(15.dp))
            }
            item {
                var likes by remember {
                    mutableStateOf(blog.likes)
                }
                var dislikes by remember {
                    mutableStateOf(blog.dislikes)
                }
                Text(
                ...
                Row(
                    ...
                    Row() {
                        ...
                        db.collection("LikeDislikes").document(blog.id).collection("uid")
                        ...
                        if (loaded) {
                            Button(onClick = {
                                ...
                                val db = FirebaseFirestore.getInstance()
                                db.collection("Blogs").document(blog.id).update("likes", likes)
                                db.collection("Blogs").document(blog.id)
                            ...
                            Button(onClick = {
                                if (pressed) {
                                    ...
                                    val data = HashMap<String, Any>()
                                    data.put("flag", 1)
                                    db.collection("LikeDislikes").document(blog.id)
                                        .collection("uid")
                                        .document(uid).set(data)
